package org.unlitrodeluzcolombia.mediamanager.gui.finder;

import java.util.LinkedHashMap;
import java.util.LinkedList;
import net.comtor.framework.html.administrable.AbstractComtorFinderFactoryI18n;
import net.comtor.framework.logic.facade.WebLogicFacade;
import org.unlitrodeluzcolombia.mediamanager.element.MusicGenre;
import org.unlitrodeluzcolombia.mediamanager.web.facade.MusicGenreWebFacade;

/**
 *
 * @author juandiego@comtor.net
 * @since Feb 18, 2019
 */
public class MusicGenreFinder
        extends AbstractComtorFinderFactoryI18n<MusicGenre, Long> {

    @Override
    protected String getValueToHide(MusicGenre genre) {
        return genre.getId() + "";
    }

    @Override
    public String getValueToShow(MusicGenre genre) {
        return "[" + genre.getId() + "] " + genre.getName();
    }

    @Override
    public WebLogicFacade<MusicGenre, Long> getFacade() {
       return new MusicGenreWebFacade();
    }

    @Override
    public LinkedHashMap<String, String> getHeaders() {
        LinkedHashMap<String, String> headers = new LinkedHashMap<>();
        headers.put("id", "ID");
        headers.put("name", "Nombre");

        return headers;
    }

    @Override
    public LinkedList<Object> getRow(MusicGenre genre) {
        LinkedList<Object> row = new LinkedList<>();
        row.add(genre.getId());
        row.add(genre.getName());

        return row;
    }

}
